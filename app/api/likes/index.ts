 ...
 */
export async function searchMainLikeProductsPage(filters: ProductSearchFilter): Promise<MainLikeProduct[]> {
  try {
    // 필터 데이터를 쿼리 파라미터로 변환
    let queryParams = '';
    ...
    const endpoint = `${API_ENDPOINTS.likes.productsOptions}?${queryParams}`;

    // GET 요청으로 쿼리 파라미터를 포함하여 전송
    return await get<MainLikeProduct[]>(endpoint);
  } catch (error) {
 ...
 * const result = await searchMainPageProducts(0, 10, filters);
 */
export async function searchMainLikeStudioPage(filters: ProductSearchFilter): Promise<MainLikeStudio[]> {
  try {
    // 필터 데이터를 쿼리 파라미터로 변환
    let queryParams = '';
    ...
    const endpoint = `${API_ENDPOINTS.likes.studiosOptions}?${queryParams}`;

    // GET 요청으로 쿼리 파라미터를 포함하여 전송
    return await get<MainLikeStudio[]>(endpoint);
  } catch (error) {
    console.error('상품 검색 실패:', error);
    throw handleApiError(error);
  }
